...
function M.grep_curbuf()
  if is_telescope() then
    t().current_buffer_fuzzy_find { default_text = vim.fn.expand "<cword>" }
  elseif is_fzf() then
    f().grep_curbuf { search = vim.fn.expand "<cword>" }
  end
end
...
function M.setup_keymaps()
  ...
  map("n", "<leader>fW", M.grep_curbuf, { desc = "[F]ind [W]ord in current buffer" })
